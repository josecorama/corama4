...
SAM_BASE_URL = "https://api.sam.gov/prod/opportunities/v1/search"
...
def fetch_sam_page(page: int) -> List[Dict[str, Any]]:
    headers = {
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            data = resp.json()
            return data.get("opportunities", []) or data.get("results", []) or data.get("data", []) or []
...
def upsert_batch(client: QdrantClient, records: List[Dict[str, Any]]):
    ...
    for rec in records:
        ...
        points.append(PointStruct(id=str(notice_id), vector=None, payload=payload))
